...
class FilteredDataset(torch.utils.data.Dataset):
    """Wrapper to filter out augmented captions from validation set"""
    def __init__(self, base_ds, keep_original_only=False):
        self.base_ds = base_ds
        self.keep_original_only = keep_original_only
    ...
    def __getitem__(self, idx):
        img, caps = self.base_ds[idx]
        if self.keep_original_only:
            # Keep only first 5 captions (original), remove last 5 (paraphrases)
            caps = caps[:5]
        return img, caps

# Training set: use all captions (original + paraphrases)
train_ds = PairDataset(train_base, tokenizer, seed=SEED)

# Validation set: use only original captions for loss computation to keep it consistent
val_base_filtered = FilteredDataset(val_base, keep_original_only=True)
val_ds = PairDataset(val_base_filtered, tokenizer, seed=SEED)
...
for epoch in range(1, EPOCHS+1):
    ...
    # —— retrieval with progress bar (use filtered validation set)
    val_ret_loader = DataLoader(
        val_base_filtered,      # Use filtered dataset for retrieval too
        batch_size=32,
        shuffle=False,
        num_workers=0,
    ...
    with torch.no_grad():
        for images, cap_lists in ret_bar:
            ...
            txt_feats = model.encode_text(toks)     # (B*5, D)  # Still 5 captions per image
    ...
    # Recall computation remains the same (still 5 captions per image)
